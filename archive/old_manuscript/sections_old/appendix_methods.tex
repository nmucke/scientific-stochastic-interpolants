...
% -------------------------------------------------------------------
\subsection{Ours (FM-SDE)}\label{appendix:method:fm_sde}
...
\cite{chen_flowdas_2025}. SI-SDE with a Monte-Carlo estimate of the likelihood score.

\textbf{SDE.}
Same SI prior SDE as above; the guidance replaces $\bar S_\tau$ with
\begin{align}
  \bar S_\tau^{\mathrm{MC}}
  = \frac{1}{J}\sum_{j=1}^{J}
    \frac{\obsmatrix^\top(\obs - \obsmatrix\hat{\state}^{1,j}_\tau)}
         {\|\obs - \obsmatrix\hat{\state}^{1,j}_\tau\|},
\end{align}
using $J=16$ sampled one-step predictions $\hat{\state}^{1,j}_\tau$ from the denoiser.

\textbf{Likelihood approximation.}
MC estimate of the denoiser-based score; residual-normalised (step-norm) for numerical stability.
No closed-form covariance inflation; the normalisation discards magnitude information.

\textbf{Key approximation.}
MC score estimate introduces variance; step-normalisation scales poorly with dimension.
Lacks the inflated source covariance of our method and the multiplicative gain $\gweight_\tau$.
...
\cite{parikh_d-flow_2026,ben-hamu_d-flow_2024}. Source-space sampling: runs an SGLD chain over the FM source latent $\bx_0\sim\mathcal{N}(0,I)$ such that $\Phi(\bx_0)=\bx_1$.

\textbf{Langevin chain (source space).}
For each of $K$ steps ($K=20$ for Navier--Stokes, $K=200$ for analytical):
\begin{align}
  \bx_0 \leftarrow \bx_0
  + \eta\,\nabla_{\bx_0}\!\left[\log\mathcal{N}(\obs;\,\obsmatrix\Phi(\bx_0),\obscov)
    + \log\mathcal{N}(\bx_0;0,I)\right]
  + \sqrt{2\eta}\,\boldsymbol\varepsilon,
  \quad \boldsymbol\varepsilon\sim\mathcal{N}(0,I),
\end{align}
where $\Phi(\bx_0)$ is the full ODE rollout from $\bx_0$. The chain is reinitialised from $\mathcal{N}(0,I)$ at $\tau=0$ for each assimilation window; does not use $\drift^{\gdiff}_\tau$ between observation windows.

\textbf{Likelihood approximation.}
Exact Gaussian likelihood $\mathcal{N}(\obs;\obsmatrix\Phi(\bx_0),\obscov)$ evaluated at the ODE endpoint.
No intermediate-time approximation; but each gradient step backpropagates through the \emph{entire} ODE rollout.

\textbf{Key approximation.}
SGLD chain may not converge in $K$ steps; gradient checkpointing bounds memory. Most expensive baseline (full trajectory backprop per step).

% -------------------------------------------------------------------
\subsection{SDA}\label{appendix:method:sda}

\cite{rozet_score-based_2023}. DPS-style observation guidance on a diffusion-model prior.

\textbf{SDE.}
Using the DM prior (from FM velocity via velocity--score identity, Appendix~\ref{appendix:score_velocity}),
with endpoint-vanishing $\gdiff_\tau$:
\begin{align}
  \rd\bXstar_\tau
  = \bigl[\fmvel_\tau + \tfrac12\gdiff_\tau^2\genscore_\tau\bigr]\rd\tau
  + \gdiff_\tau^2\,\nabla_{\bXstar_\tau}\log\mathcal{N}\!\bigl(\obs;\,\obsmatrix\hat{\state}^1_\tau,\,
    \sigma^2 I + \gdiff_\tau^2\,\obsmatrix\obsmatrix^\top\bigr)\,\rd\tau
  + \gdiff_\tau\,\rd\bW_\tau.
\end{align}
Tweedie denoiser: $\hat{\state}^1_\tau = \bXstar_\tau - \gdiff_\tau^2\genscore_\tau(\bXstar_\tau)/\gdiff_\tau^2\beta_\tau$ (from the prior marginal).

\textbf{Likelihood approximation.}
Approximates $p(\bx_1|\bXstar_\tau)\approx\mathcal{N}(\hat{\state}^1_\tau,\,\sigma^2 I)$ via Tweedie (diagonal-plus-low-rank covariance $\sigma^2 I + \gdiff_\tau^2\obsmatrix\obsmatrix^\top$). Guidance weight is $\gdiff_\tau^2$ (Doob $h$-transform), not the marginal-path weight $\gweight_\tau$.

\textbf{Key approximation.}
Uses Doob $h$-transform weight $\gdiff_\tau^2$ rather than $\gweight_\tau=\vscoef_\tau+\tfrac12\gdiff_\tau^2$; intermediate marginals are not $p_\tau^{\obs}$.
Isotropic Tweedie covariance; no source-covariance inflation from the model.

% -------------------------------------------------------------------
\subsection{SURGE}\label{appendix:method:surge}

\cite{wei_surge_2026}. Sequential Monte-Carlo on the DM prior: reweight particles with $p(\obs|\bXstar_\tau)$ and resample, then continue with the prior SDE.

\textbf{Algorithm.}
Denote particle $i$ at pseudo-time $\tau$ by $\bx_\tau^{(i)}$.
At each $\tau$-step for each particle $i\in\{1,\ldots,N\}$:
\begin{enumerate}
  \item Advance with prior SDE: $\bx_\tau^{(i)} \leftarrow \bx_{\tau-\Delta\tau}^{(i)}
        + \bigl[\fmvel_\tau+\tfrac12\gdiff_\tau^2\genscore_\tau\bigr]\Delta\tau
        + \gdiff_\tau\,\sqrt{\Delta\tau}\,\boldsymbol\varepsilon^{(i)}$.
  \item Compute importance weights $\tilde{w}^{(i)} \propto p\!\bigl(\obs\mid\bx_\tau^{(i)}\bigr)$.
  \item If $\mathrm{ESS} = \bigl(\sum_i (\tilde{w}^{(i)})^{2}\bigr)^{-1} < 0.5N$: resample particles.
\end{enumerate}
No gradient of the likelihood required.

\textbf{Likelihood approximation.}
Importance weights use the \emph{marginal} $p(\obs|\bXstar_\tau)$, not the intermediate tilt $\Phi_\tau^{\obs}$.
In practice approximated by a Gaussian at $\hat{\state}^1_\tau$ (DPS-style guidance is used to improve the proposal).

\textbf{Key approximation.}
Particle resampling causes degeneracy in high dimension; ESS threshold $=0.5N$.
Does not use the observation interpolant or source-covariance inflation.
...
SDA, SURGE, and FM-SDE require a diffusion prior.
