...
\textbf{Likelihood approximation.}
Same interpolant-likelihood as SI-SDE with FM anchor coefficient $\alpha_\tau\equiv0$:
$\interpolantobs_\tau = \beta_\tau\obs$, and FM source moments $\srcmean_\tau=-\sigpath_\tau^2\genscore_\tau$,
$\srccov_\tau\approx\sigpath_\tau^2 I$ (Jacobian-free). The guidance weight reduces to $\gweight_\tau=\vscoef_\tau$ (no diffusion term).

\textbf{Key approximation.}
Gaussian surrogate for $\Phi_\tau^{\obs}$; inflated covariance $\beta_\tau^2\obscov+\sigpath_\tau^2\obsmatrix\obsmatrix^\top$.
...
\textbf{SDE.}
With endpoint-vanishing diffusion $\gdiff_\tau\propto\sqrt{\sigpath_\tau\beta_\tau}$ and lifted drift $\fmvel_\tau+\tfrac12\gdiff_\tau^2\genscore_\tau$:
\begin{align}
  \rd\bXstar_\tau
...
\textbf{Likelihood approximation.}
Identical to FM-ODE: $\interpolantobs_\tau=\beta_\tau\obs$, inflated covariance $\beta_\tau^2\obscov+\sigpath_\tau^2\obsmatrix\obsmatrix^\top$.
...
\textbf{Cost.}
Each pseudo-time step needs \textbf{two forward passes} through the network -- the prior drift $\fmvel_\tau(\state_\tau)$ (also used for the drift update) and the Heun endpoint $\fmvel_1$ for the one-step denoiser $\hat{\state}^1_\tau$ -- and \textbf{one backward pass} (differentiating $\bar S_\tau^{\mathrm{MC}}$ through $\hat{\state}^1_\tau$). This is independent of $J$: the $J$ predictions share the single denoiser mean and add only cheap linear $\obsmatrix$ evaluations, not network passes.
...
\textbf{Cost.}
Each pseudo-time step needs \textbf{one forward pass} through the network -- the prior velocity $\fmvel_\tau(\bXstar_\tau,\state^0)$, reused unchanged in the one-step denoiser $\hat{\state}^1_\tau=\bXstar_\tau+(1-\tau)\fmvel_\tau$ -- and \textbf{no backward pass}: the corrector differentiates the residual norm $\|\obs-\obsmatrix\bXstar_\tau\|$ through the linear observation operator $\obsmatrix$ only, so each of the $k$ inner corrections costs a single $\obsmatrix,\obsmatrix^\top$ evaluation rather than a network gradient. FIG is thus the cheapest guided sampler in this comparison -- the only one needing neither a second network forward nor any network backward -- independent of $k$ and of the ensemble size.
...
$\Phi$ is the FM-ODE rollout, integrated with a $6$-step midpoint (RK2) scheme and backpropagated in full (gradient-checkpointed). The data term is the plain squared residual (no $1/2\obscov$ weighting); the source-penalty weight $\lambda$ sets the data--prior balance and is the dominant hyperparameter, tuned per scenario (\cite{parikh_d-flow_2026}, Table~D.3).

\textbf{Cost.}
Each pseudo-time step of the transport rollout needs \textbf{two forward passes} (the two midpoint/RK2 velocity stages) and, because the data misfit is differentiated through the whole flow, \textbf{two backward passes}. Unlike the guided samplers, which integrate their $M$ pseudo-time steps \emph{once}, D-Flow SGLD repeats the full $S=6$-step rollout at every one of its $K$ pSGLD steps: $2S=12$ forward and $12$ backward network passes per pSGLD step (the forward count doubled by gradient checkpointing, which recomputes the rollout during the backward), plus one forward-only reconstruction per sample. It is the only baseline with backward passes through the network, and by far the most expensive.
...
\textbf{Cost.}
Each pseudo-time step needs \textbf{two forward passes} through the network -- one for the prior drift $\fmvel_\tau+\tfrac12\gdiff_\tau^2\genscore_\tau$ and one for the Tweedie denoiser $\hat{\state}^1_\tau$ inside the guidance term -- and \textbf{one backward pass}, the DPS gradient $\nabla_{\bXstar_\tau}\log\mathcal{N}(\obs;\obsmatrix\hat{\state}^1_\tau,\cdot)$ differentiated through $\hat{\state}^1_\tau$. The two forwards evaluate the same velocity network at $(\bXstar_\tau,\tau)$ but are not shared: the prior-drift forward is detached, while the guidance forward must retain its graph for the backward. This backward is what makes SDA costlier per step than the closed-form interpolant samplers (SI-SDE/DM-SDE/FM-ODE), whose guidance $\bar S_\tau$ needs no network gradient.
...
\textbf{Cost.}
On top of the wrapped method, SURGE adds only the particle weighting: the reward $R_\tau$ needs the one-step denoiser $\hat{\state}^1_\tau$ at the current and the proposed state -- \textbf{two forward passes per pseudo-time step and no backward pass} (the Girsanov terms are closed-form and resampling is $O(N)$). The current-state denoiser coincides with the one the wrapped guidance already forms, so the strictly-added work is a single denoiser pass per step. The overhead is thus a fixed additive cost -- independent of the ensemble size $N$, of the wrapped method, and of FlowDAS's $J$ -- and leaves the step count $M$ and number of windows unchanged.
