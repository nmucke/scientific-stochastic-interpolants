...
% \section{Conditioning the affine Gaussian path: proofs}
% \label{appendix:proof_conditioning}

% This appendix proves Proposition~\ref{prop:conditional_path} and Theorem~\ref{theorem:unified_posterior}, the two results that turn the prior affine Gaussian path into the unified posterior sampler. Both reuse the structural facts of Appendix~\ref{appendix:score_velocity}: the velocity--score duality \eqref{eq:general_velocity_of_score} and the marginal-preserving SDE family (Proposition~\ref{prop:marginal_preservation}).

% \subsection{Proof of Proposition~\ref{prop:conditional_path}}
% \begin{proof}
% Recall the prior interpolant \eqref{eq:general_interpolant}, $\bx_\tau = \alpha_\tau\bm a_0 + \sigpath_\tau\latentnoise + \beta_\tau\bx_1$, with $\latentnoise\sim\mathcal{N}(0,\bI)$ independent of $\bx_1$, and the observation $\obs = \obsoperator(\bx_1) + \obsnoise$ with $\obsnoise$ independent of $(\bx_1,\latentnoise)$. Conditioning the joint law of $(\bx_1,\latentnoise)$ on $\obs$ leaves the Gaussian driver unchanged: since $\latentnoise$ is independent of $(\bx_1,\obsnoise)$ and $\obs$ is a function of $(\bx_1,\obsnoise)$, we have $\latentnoise \perp \obs \mid \bx_1$, hence
% \begin{align}
%     \conddist{\latentnoise}{\bx_1,\obs} = \conddist{\latentnoise}{\bx_1} = \mathcal{N}(0,\bI),
% \end{align}
% so $\latentnoise\mid\obs\sim\mathcal{N}(0,\bI)$ and remains independent of $\bx_1\mid\obs$. The conditional law of the interpolant is therefore again the affine Gaussian path
% \begin{align}
%     \bx_\tau = \alpha_\tau\bm a_0 + \sigpath_\tau\latentnoise + \beta_\tau\bx_1,
%     \qquad \bx_1\sim\conddist{\bx_1}{\obs,\bx_0},\ \ \latentnoise\sim\mathcal{N}(0,\bI),
% \end{align}
% with the \emph{same} schedules $(\alpha_\tau,\beta_\tau,\sigpath_\tau)$ and anchor $\bm a_0$, and with the target retargeted to the posterior $\conddist{\bx_1}{\obs,\bx_0}$.

% For the score, Bayes' rule on the marginals gives $p_\tau^{\obs}(\bx\mid\bx_0)\propto \Phi_\tau^{\obs}(\bx,\bx_0)\,p_\tau(\bx\mid\bx_0)$, where $\Phi_\tau^{\obs}(\bx,\bx_0)=\conddist{\obs}{\bx_\tau=\bx,\bx_0}=\E[\conddist{\obs}{\bx_1}\mid\bx_\tau=\bx,\bx_0]$ since $\obs\perp\bx_\tau\mid\bx_1$. Taking $\nabla_{\bx}\log$ yields \eqref{eq:conditional_score},
% \begin{align}
%     \genscore_\tau^{\obs}(\bx,\bx_0) = \genscore_\tau(\bx,\bx_0) + \nabla_{\bx}\log\Phi_\tau^{\obs}(\bx,\bx_0).
% \end{align}
% For the velocity, apply the velocity--score duality \eqref{eq:general_velocity_of_score} to the posterior path. Because the posterior path has the same schedules and anchor as the prior path, the affine-in-$\bx$ part of \eqref{eq:general_velocity_of_score} is identical, so
% \begin{align}
%     \fmvel_\tau^{\obs}(\bx)
%     &= \frac{\dot\beta_\tau}{\beta_\tau}\bx + \left(\dot\alpha_\tau - \frac{\dot\beta_\tau\alpha_\tau}{\beta_\tau}\right)\bm a_0 + \vscoef_\tau\,\genscore_\tau^{\obs}(\bx), \\
%     \fmvel_\tau(\bx)
%     &= \frac{\dot\beta_\tau}{\beta_\tau}\bx + \left(\dot\alpha_\tau - \frac{\dot\beta_\tau\alpha_\tau}{\beta_\tau}\right)\bm a_0 + \vscoef_\tau\,\genscore_\tau(\bx).
% \end{align}
% Subtracting and using \eqref{eq:conditional_score} gives \eqref{eq:conditional_velocity},
% \begin{align}
%     \fmvel_\tau^{\obs}(\bx) - \fmvel_\tau(\bx)
%     = \vscoef_\tau\!\left(\genscore_\tau^{\obs}(\bx) - \genscore_\tau(\bx)\right)
%     = \vscoef_\tau\,\nabla_{\bx}\log\Phi_\tau^{\obs}(\bx,\bx_0). \qedhere
% \end{align}
% \end{proof}
...
% \subsection{Proof of Theorem~\ref{theorem:unified_posterior}}
% \begin{proof}
% By Proposition~\ref{prop:conditional_path} the posterior marginals $p_\tau^{\obs}$ are those of an affine Gaussian path with velocity $\fmvel_\tau^{\obs}$ and score $\genscore_\tau^{\obs}$. Applying the marginal-preserving SDE family of Proposition~\ref{prop:marginal_preservation} to this path, for any $\gdiff_\tau\ge0$ the SDE
% \begin{align}
%     \rd\bXstar_\tau = \left[\fmvel_\tau^{\obs}(\bXstar_\tau,\bx_0) + \tfrac12\gdiff_\tau^2\,\genscore_\tau^{\obs}(\bXstar_\tau,\bx_0)\right]\rd\tau + \gdiff_\tau\,\rd\bW_\tau,
% \end{align}
% started from $\bXstar_0\sim p_0^{\obs}$, has marginal law $p_\tau^{\obs}$ for all $\tau\in[0,1]$; in particular $\bXstar_1\sim p_1^{\obs}=\conddist{\bx_1}{\obs,\bx_0}$. The initial law $p_0^{\obs}$ is the $\tau=0$ marginal of the posterior path, which by \eqref{eq:posterior_marginal} is $p_0^{\obs}(\rd\bx)\propto\Phi_0^{\obs}(\bx,\bx_0)\,p_0(\rd\bx)$. Substituting the conditional velocity and score \eqref{eq:conditional_score}--\eqref{eq:conditional_velocity} gives the drift-correction form \eqref{eq:unified_posterior_drift} with guidance weight $\gweight_\tau=\vscoef_\tau+\tfrac12\gdiff_\tau^2$.

% The initial reweighting is in fact vacuous for both model classes. Because $\beta_0=0$, the source $\bx_{\tau=0}=\alpha_0\bm a_0+\sigpath_0\latentnoise$ does not depend on $\bx_1$, so under the interpolant coupling $\latentnoise\perp\bx_1$ the tilt $\Phi_0^{\obs}(\bx,\bx_0)=\E[\conddist{\obs}{\bx_1}\mid\bx_{\tau=0}=\bx,\bx_0]=\conddist{\obs}{\bx_0}$ is constant in $\bx$. Hence $p_0^{\obs}=p_0$: for flow matching the exact initialisation is simply $\mathcal{N}(0,\bI)$, and for stochastic interpolants the point-mass source $\delta_{\bx_0}$ is unchanged. This is the marginal construction; the situation differs for the path-space construction of Appendix~\ref{appendix:proof_posterior}, where the tilt is propagated along the Markov SDE and $\Phi_0^{\obs}$ is genuinely state-dependent, so the reweighting there is required.
% \end{proof}

% \begin{remark}[Degeneracy of the deterministic member for stochastic interpolants]
% For $\gdiff_\tau=0$ the SDE reduces to the probability-flow ODE of the posterior path, which transports $p_0^{\obs}$ to $p_1^{\obs}$ deterministically. This requires a non-degenerate source: for flow matching $p_0=\mathcal{N}(0,\bI)$ and the map carries the posterior spread, whereas for stochastic interpolants $p_0=\delta_{\bx_0}$ is a point mass and the deterministic flow would map it to a single point. The guided ODE is therefore available for flow matching but not for stochastic interpolants, whose randomness is supplied only through the diffusion term and is lost at $\gdiff_\tau=0$.
% \end{remark}
