\section{Comparison to alternative methods}\label{appendix:comparison}

Our method shares building blocks with existing approaches but differs in how it assembles them; quantitative comparisons are in Section~\ref{sec:results}, and the per-method implementation details in Appendix~\ref{appendix:methods}.

\paragraph{FlowDAS.}
FlowDAS \cite{chen_flowdas_2025}, the closest prior method, commits to a single SI-SDE and estimates the likelihood score by Monte-Carlo over sampled trajectories. We instead read it off the closed-form observation interpolant \eqref{eq:interpolated_observation} with the inflated covariance \eqref{eq:interpolant_score_closed}, removing both sampling variance and per-step trajectory backprop, and freeing $\gdiff_\tau$ so the SI-SDE becomes one member of a family also containing the DM-SDE and guided ODE. The trade-off: our likelihood is a Gaussian surrogate (Section~\ref{subsec:obs_interpolation}), its bias controlled by the covariance choice rather than the number of samples.
...
The Gaussian likelihood surrogate originates in diffusion guidance: DPS \cite{chung_diffusion_2023} uses the denoiser mean with uninflated covariance, $\Pi$GDM \cite{song_pseudoinverse_2023} inflates it. We adopt the inflated ($\Pi$GDM-style) covariance \eqref{eq:interpolant_score_closed} as our likelihood-score covariance, which DPS would leave uninflated: the inflation is markedly more accurate for sparse observations (Section~\ref{sec:results}), at the cost of an $N_y\times N_y$ solve, while the cheap Jacobian-free covariance (Corollary~\ref{cor:cheap_drift}) suffices when observations are dense.
