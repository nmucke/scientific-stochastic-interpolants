...
\begin{figure*}[ht]
    ...
    \caption{Source-covariance approximation ablation for our three samplers on the analytical linear--Gaussian case: KL to the exact posterior versus the number of sampler steps $M$, for the per-member (exact) Jacobian, the shared ensemble-mean Jacobian, and the isotropic Jacobian-free covariance (Corollary~\ref{cor:cheap_drift}). All three samplers reach the exact posterior (KL $\approx10^{-3}$) with either the exact or the shared Jacobian, while the isotropic Jacobian-free approximation plateaus at a step-independent bias (KL $\approx0.9$--$1.4$). In this linear--Gaussian case the source covariance is state-independent, so the per-member and shared Jacobians \emph{coincide exactly} (their curves overlap); the per-member-vs-shared distinction only matters for a nonlinear prior (the Navier--Stokes and urban cases).}
...
On the Navier--Stokes case we isolate the effect of the components introduced in Section~\ref{sec:methodology}. We vary: (i) the covariance along the cost/accuracy axis of Section~\ref{subsec:approximations} -- the Jacobian-free isotropic covariance of Corollary~\ref{cor:cheap_drift} (\texttt{dps\_jacobian\_free}) versus the ensemble-shared inflated source covariance (\texttt{inflated-shared}); (ii) the diffusion strength $\gdiff_\tau$ for the FM-SDE, to map the accuracy/calibration trade-off between the deterministic ODE ($\gdiff_\tau=0$) and strong-diffusion limits; (iii) the number of integration steps $M$; and (iv) the ensemble size $E$. Table~\ref{tab:ablation} collects the results.

\emph{Expected narrative (ablation run in progress).} The cells of Table~\ref{tab:ablation} are marked \texttt{--} while the ablation job runs; we state the outcome the construction predicts. The covariance axis should trade off with observation density, mirroring the two headline findings: the inflated-shared covariance helps most under \emph{sparse} observations, where the prior-covariance structure carries information across unobserved regions, whereas under \emph{dense} observations the cheap Jacobian-free covariance already suffices and the inflation adds little. Increasing the diffusion strength $\gdiff_\tau$ (moving from the ODE toward stronger-diffusion SDEs) should improve calibration -- better spread--skill and flatter rank histograms -- at fixed point accuracy, consistent with the role of the equal-marginal noise. Point accuracy should saturate in $M$ (diminishing returns past a moderate step count, the discretisation error already being small), and the ensemble spread should improve with $E$ as the empirical source covariance and the shared-Jacobian approximation tighten toward their population limits.
...
Jacobian-free corollary (\texttt{dps\_jacobian\_free}) -- for each of our three samplers. The
individual and shared covariances coincide to within seed scatter (KL $\approx 10^{-3}$): the source
covariance is \emph{state-independent} for a linear prior, so sharing the Jacobian across the
ensemble introduces no error, and the shared/individual gap only appears for a nonlinear prior (the
Navier--Stokes case). Dropping the curvature entirely (Jacobian-free isotropic) is
$\sim\!30$--$50\times$ worse (KL $\approx 0.04$--$0.07$); the sliced-$W_2$ distances follow the same
pattern (individual/shared $0.016$--$0.022$, Jacobian-free $0.12$--$0.16$). This is the analytical
counterpart of the Navier--Stokes covariance ablation above, with the exact posterior as reference.

\begin{table}[ht]
    \centering
    \small
    \caption{Analytical covariance-approximation ablation: KL divergence to the exact posterior (lower is better), averaged over seeds, per sampler, for the three likelihood-covariance modes. Individual $\approx$ shared (the state-independent covariance makes sharing exact); the isotropic Jacobian-free mode loses accuracy. Sliced-$W_2$ follows the same ordering.}
    \label{tab:analytical_ablation}
    \begin{tabular}{l|ccc}
        \toprule
        Covariance mode & SI-SDE & FM-SDE & FM-ODE \\
        \midrule
        Individual Jacobian (\texttt{inflated})        & 0.001 & 0.002 & 0.001 \\
        Shared Jacobian (\texttt{inflated-shared})     & 0.002 & 0.001 & 0.001 \\
        Jacobian-free isotropic (\texttt{dps\_jac.\,free}) & 0.041 & 0.048 & 0.073 \\
        \bottomrule
    \end{tabular}
\end{table}
