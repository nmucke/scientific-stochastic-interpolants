#!/bin/bash
# =============================================================================
# Navier--Stokes M=25 fill -- PARALLEL CPU driver, NO D-FLOW  (2026-07-20)
#
# Same scheduler as run_ns_grid_M25_parallel.sh, pointed at the grid script that
# leaves D-Flow SGLD out (run_ns_grid_M25_nodflow.sh). Rationale, from the measured
# 2026-07-16 run: D-Flow SGLD is ~4.7 h per assimilation step => ~71 h/cell, versus
# ~20 h for the other five baselines together. Dropping it cuts the fill from
# ~25 days to ~5-6. Run D-Flow separately afterwards (GPU preferred):
#   GRPS=dflow DEVICE=cuda bash paper_experiments/run_ns_grid_M25_nodflow.sh
#
# Own LOGDIR (logs_M25_nodflow/), so this never interleaves with the older
# logs_M25/ shard logs.
#
# Fans run_ns_grid_M25_nodflow.sh out across (traj x scenario) shards on the CPU. This is
# a pure scheduler: every shard is just run_ns_grid_M25_nodflow.sh with TRAJ/SCENARIOS
...
#
# Launch detached:
#   setsid nohup bash paper_experiments/run_ns_grid_M25_nodflow_parallel.sh \
#     >run_ns_grid_M25_nodflow_parallel.log 2>&1 & disown
#
# Track:
#   tail -f paper_experiments/results/navier_stokes/logs_M25_nodflow/shard_*.log
#   .venv/bin/python paper_experiments/status.py
#   grep -c OK paper_experiments/results/navier_stokes/logs_M25_nodflow/*.log
#
...
LOGDIR="$ROOT/logs_M25_nodflow"
mkdir -p "$LOGDIR"

WORKERS="${WORKERS:-3}"                     # concurrent shards -- RAM-bound (~18 GB/shard)
THREADS="${THREADS:-$((32 / WORKERS))}"     # torch threads PER shard (32/3 = 10)
# 18 GB is the measured peak WITH D-Flow SGLD, which is the memory hog (it backprops
# through the sampler). Without it the remaining five baselines should peak well
# below that, so WORKERS can probably go to 5-6 -- but that is UNMEASURED. Watch RSS
# on a first pass, then raise PEAK_GB/WORKERS; the pre-flight check below is what
# stands between an overcommit and the OOM-killer, so do not bypass it with FORCE=1.
...
run_shard() {
  ...
  # says `none`, which the guard in run_ns_grid_M25_nodflow.sh accepts as "save nothing".
  ...
  OMP_NUM_THREADS="$THREADS" MKL_NUM_THREADS="$THREADS" \
    bash paper_experiments/run_ns_grid_M25_nodflow.sh >>"$log" 2>&1
