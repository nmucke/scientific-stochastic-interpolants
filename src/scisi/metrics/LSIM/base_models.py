...
class LSiM_Base(torch.nn.Module):
    ...
    def forward(self, X):
        ...
        outputs = namedtuple(
            "LSiMBaseOutputs", ["relu1", "relu2", "relu3", "relu4", "relu5"]
        )
...
class DfpNet(nn.Module):
    def __init__(self, pretrained=True, channelExponent=6, dropout=0.0):
    ...
    def forward(self, x):
        ...
        outputs = namedtuple(
            "DfpNetOutputs",
            ["block1", "block2", "block2x", "block3", "block4", "block5", "block6"],
        )
        out = outputs(
            h_block1, h_block2, h_block2x, h_block3, h_block4, h_block5, h_block6
        )
