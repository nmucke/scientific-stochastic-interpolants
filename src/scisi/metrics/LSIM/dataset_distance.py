...
class DatasetDistance(Dataset):
    def __init__(
        self, name, dataDirs, exclude=[], include=[], transform=None, fileType="png"
    ):
    ...
    def __getitem__(self, idx):
        ...
        for fileName in fileNames:
            ...
            if (
                not fileName.endswith(".%s" % self.fileType)
                or fileName == "ref.%s" % self.fileType
            ):
                continue
        ...
        assert len(listDist) != 0 and len(listFrames) != 0, (
            "%s: no data to load!" % directory
        )
        ...
        sample = {
            "reference": np.stack(reference, 0),
            "other": np.stack(other, 0),
            "distance": np.stack(dist, 0),
            "path": directory,
        }
...
class TransformsTrain(object):
    ...
    def __call__(self, sample):
        ...
        resultRef = np.zeros(
            [reference.shape[0], self.outputSize, self.outputSize, reference.shape[3]]
        )
        resultOther = np.zeros(
            [other.shape[0], self.outputSize, self.outputSize, other.shape[3]]
        )
        ...
        for i in range(reference.shape[0]):
            ...
            resultRef[i] = ref[
                top : top + self.outputSize, left : left + self.outputSize
            ]
            resultOther[i] = oth[
                top : top + self.outputSize, left : left + self.outputSize
            ]
        ...
        # normalization
        dMin = np.minimum(
            np.min(resultRef, axis=(0, 1, 2)), np.min(resultOther, axis=(0, 1, 2))
        )
        dMax = np.maximum(
            np.max(resultRef, axis=(0, 1, 2)), np.max(resultOther, axis=(0, 1, 2))
        )
        if (dMin == dMax).all():
            ...
            for i in range(dMin.shape[0]):
                if dMin[i] == dMax[i]:
                    ...
                    resultRef[..., i] = (self.normMax - self.normMin) * (
                        (resultRef[..., i] - dMin[i]) / (dMax[i] - dMin[i])
                    ) + self.normMin
                    resultOther[..., i] = (self.normMax - self.normMin) * (
                        (resultOther[..., i] - dMin[i]) / (dMax[i] - dMin[i])
                    ) + self.normMin
        else:
            resultRef = (self.normMax - self.normMin) * (
                (resultRef - dMin) / (dMax - dMin)
            ) + self.normMin
            resultOther = (self.normMax - self.normMin) * (
                (resultOther - dMin) / (dMax - dMin)
            ) + self.normMin
...
class TransformsInference(object):
    ...
    def __call__(self, sample):
        ...
        # resize
        if self.outputSize and (
            self.outputSize != reference.shape[1]
            or self.outputSize != reference.shape[2]
        ):
            resultRef = np.zeros(
                [
                    reference.shape[0],
            ...
            zoom1 = [
                1,
                self.outputSize / reference.shape[1],
                self.outputSize / reference.shape[2],
                1,
            ]
            resultRef = scipy.ndimage.zoom(reference, zoom1, order=self.order)
            zoom2 = [
                1,
                self.outputSize / other.shape[1],
                self.outputSize / other.shape[2],
                1,
            ]
