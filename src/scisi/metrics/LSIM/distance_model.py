...
from scisi.metrics.LSIM.base_models import *
from scisi.metrics.LSIM.dataset_distance import *
...
class DistanceModel(nn.Module):
    ...
    def __init__(
        ...
        self.lin0 = self.linearLayer(
            self.basenet.channels[0], self.basenet.featureMapSize[0]
        )
        self.lin1 = self.linearLayer(
            self.basenet.channels[1], self.basenet.featureMapSize[1]
        )
        self.lin2 = self.linearLayer(
            self.basenet.channels[2], self.basenet.featureMapSize[2]
        )
        self.lin3 = self.linearLayer(
            self.basenet.channels[3], self.basenet.featureMapSize[3]
        )
        self.lin4 = self.linearLayer(
            self.basenet.channels[4], self.basenet.featureMapSize[4]
        )
        self.linear = [self.lin0, self.lin1, self.lin2, self.lin3, self.lin4]
        if baseType in ["squeeze", "dfp", "lsimSkip"]:
            self.lin5 = self.linearLayer(
                self.basenet.channels[5], self.basenet.featureMapSize[5]
            )
            self.lin6 = self.linearLayer(
                self.basenet.channels[6], self.basenet.featureMapSize[6]
            )
            self.linear = self.linear + [self.lin5, self.lin6]
            if baseType == "lsimSkip":
                self.lin7 = self.linearLayer(
                    self.basenet.channels[7], self.basenet.featureMapSize[7]
                )
    ...
    def computeDistance(self, input1, input2, interpolateTo=224, interpolateOrder=0):
        assert (
            not self.training
        ), "Distance computation should happen in evaluation mode!"
        assert (
        ...
        data_transform = TransformsInference(
            interpolateTo, interpolateOrder, normMin=0, normMax=255
        )
    ...
    def normalizeTensor(self, tensorIn, layer, epsilon=1e-10, updateAcc=False):
        ...
        # over all samples of all batches
            ...
            if updateAcc:
                ...
                self.normAcc[layer] = self.normAcc[layer].detach() + torch.sum(
                    torch.mean(delta / self.normCount[layer], dim=1), dim=0
                )
                self.normM2[layer] = self.normM2[layer].detach() + torch.sum(
                    torch.mean(
                        delta
                        * (tensorIn - self.normAcc[layer].detach().expand_as(tensorIn)),
                        dim=1,
                    ),
                    dim=0,
                )
            ...
            if (
                size[2] != self.normAcc[layer].shape[0]
                or size[3] != self.normAcc[layer].shape[1]
            ):
                up = nn.Upsample(
                    size=(size[2], size[3]), mode="bilinear", align_corners=True
                )
                normAcc = torch.squeeze(
                    up(
                        torch.unsqueeze(
                            torch.unsqueeze(self.normAcc[layer].detach(), dim=0), dim=0
                        )
                    )
                )
                normM2 = torch.squeeze(
                    up(
                        torch.unsqueeze(
                            torch.unsqueeze(self.normM2[layer].detach(), dim=0), dim=0
                        )
                    )
                )
            ...
            # directly use norm accumulators for input size 224x224
                ...
                mean = mean.view(1, 1, size[2], size[3])
                std = torch.sqrt(
                    self.normM2[layer].detach() / (self.normCount[layer] - 1)
                )
                std = std.view(1, 1, size[2], size[3])
            normalized = (tensorIn - mean.expand_as(tensorIn)) / (
                std.expand_as(tensorIn) + epsilon
            )
