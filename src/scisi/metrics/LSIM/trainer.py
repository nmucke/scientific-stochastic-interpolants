    ...
    def trainingStep(self, epoch):
        ...
        for i, sample in enumerate(self.trainLoader, 0):
            ...
            if i % 6 == 0 and self.showProgressPrint:
                print(
                    "Training Epoch:",
                    round((100 * i) / len(self.trainLoader), 1),
                    "%",
                    end="\r",
                )
            ...
            if i % self.printEvery == self.printEvery - 1:
                ...
                print(
                    "%3ds [%2d, %4d] loss: %.5f(%.3f + %.3f) cor: %.3f"
                    % (
                        end - start,
                        epoch,
                        i + 1,
        ...
        if self.showProgressPrint:
            print(
                "                                                          ", end="\r"
            )  # override temporary percentage print
                ...
                for i, sample in enumerate(self.trainLoader, 0):
                    ...
                    if i % 8 == 0 and self.showProgressPrint:
                        print(
                            "Norm calibration:",
                            round(
                                (100 * (i + epoch * len(self.trainLoader)))
                                / (epochs * len(self.trainLoader)),
                                1,
                            ),
                            "%",
                            end="\r",
...
class Validator(object):
    ...
    def validationStep(self):
        ...
        with torch.no_grad():
            ...
            print(
                "%ds Validation (distance error mean/std) (correlation mean/std):"
                % (end - start)
            )
            print(
                "%1.3f (%1.3f) -- %1.4f (%1.4f)" % (distMean, distStd, corMean, corStd)
            )
            print()
    ...
    def save(self, path):
        ...
        histDict = {
            "histDistMean": self.histDistMean,
