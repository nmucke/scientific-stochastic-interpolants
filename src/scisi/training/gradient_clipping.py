...
class EmaGradientClipper:
    ...
    def _record_norm(self, new_norm: float) -> None:
        self.ema_index += 1
        self._grad_norm_ema1 = (
            self.ema_coef1 * self._grad_norm_ema1 + (1 - self.ema_coef1) * new_norm
        )
        self._grad_norm_ema2 = (
            self.ema_coef2 * self._grad_norm_ema2 + (1 - self.ema_coef2) * new_norm
        )

    @property
    def _current_ema1(self) -> float:
        return self._grad_norm_ema1 / (1 - self.ema_coef1**self.ema_index)

    @property
    def _current_ema2(self) -> float:
        return self._grad_norm_ema2 / (1 - self.ema_coef2**self.ema_index)

    def clip_grads(self, parameters: Iterable[torch.Tensor]) -> None:
        ...
        norm = (
            self.clip_norm_ratio * self._current_ema1
            if clipped and self.ema_index != 0
            else total_norm
        )
